...
def beam_search(
  ...
  Args:
    ...
    cache_offset: axis offset for cache, arising from scanned layers.
    logit_callback_fn: function to modify the logits at each step
  Returns:
     Tuple of:
  ...
  # Checking decode_until < max_decode_len breaks jitted code since
  # max_decode_len is used to construct the static graph.
  ...
  def beam_search_loop_cond_fn(state):
    ...
    not_at_end = (state.cur_index < decode_until - 1)
  ...
  def beam_search_loop_body_fn(state):
    ...
    if logit_callback_fn is not None:
      logits = logit_callback_fn(logits, state.cur_index)
